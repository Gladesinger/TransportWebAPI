...
//! Front-end processor
class Vehicle_capacity extends Controller {

	//! GET all method
	function GetAll($f3) {
		$db = $this->db;

		$res = $db->exec('select * from directory.vehicle_capacity_class ORDER BY id');
	...
	function GetOne($f3) {
		...
		$res = $db->exec('select * from directory.vehicle_capacity_class WHERE id = ?', $f3->get('PARAMS.id'));
	...
	function Add($f3) {
		...
		$object = new DB\SQL\Mapper($db,'directory.vehicle_capacity_class');
