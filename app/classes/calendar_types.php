...
//! Front-end processor
class Calendar_types extends Controller {

	//! GET all method
	function GetAll($f3) {
		$db = $this->db;

		$res = $db->exec('select * from directory.calendar_types ORDER BY id');
	...
	function GetOne($f3) {
		...
		$res = $db->exec('select * from directory.calendar_types WHERE id = ?', $f3->get('PARAMS.id'));
	...
	function Add($f3) {
		...
		$object = new DB\SQL\Mapper($db,'directory.calendar_types');
