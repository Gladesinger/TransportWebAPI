...
class Vehicle_brand extends Controller {
	...
	function GetAll($f3) {
		...
		$res = $db->exec('select * from directory.vehicle_brand ORDER BY id');
	...
	function GetOne($f3) {
		...
		$res = $db->exec('select * from directory.vehicle_brand WHERE id = ?', $f3->get('PARAMS.id'));
	...
	function Add($f3) {
		...
		$object = new DB\SQL\Mapper($db,'directory.vehicle_brand');

		$object->name = $body['name'];
		$object->consumption_rate = $body['consumption_rate'];
	...
	function Update($f3) {
		...
		if($body['consumption_rate'] != null){
			$object->code = $body['consumption_rate'];
		}
