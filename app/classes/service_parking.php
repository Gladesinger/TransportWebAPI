...
//! Front-end processor
class Service_parking extends Controller {

	//! GET all method
	function GetAll($f3) {
		$db = $this->db;

		$res = $db->exec('select * from directory.service_parking ORDER BY id');
	...
	function GetOne($f3) {
		...
		$res = $db->exec('select * from directory.service_parking WHERE id = ?', $f3->get('PARAMS.id'));
	...
	function Add($f3) {
		...
		$object = new DB\SQL\Mapper($db,'directory.service_parking');
